...
import sys
sys.path.append('../../')
from dim_pucker.utils import complementary
...
#     for tetramer in get_all_tetramers(complementary(seq)):
#         if not(tetramer in dic_tetramers):
#             dic_tetramers[tetramer] = []
...
# logistic_regression_kwargs={'fit_intercept': True, 'penalty': 'l1', 'C': 1.0, 'tol': 0.0000001, 'solver': 'saga', 'max_iter': 500}
k = 2
for ci in c:
    ...
    logistic_regression_kwargs={'fit_intercept': True, 'penalty': 'l1', 'C': ci, 'tol': 0.0000001, 'solver': 'saga', 'max_iter': 500}

    for seq_no, seq in enumerate(list_labels):
        full_seq = 'GC'+seq[2:]+seq*3+'GC' + complementary('GC'+seq[2:]+seq*3+'GC') # 5-3 seq + 5-3 comp_seq
        ...
        for i in range(int((len(full_seq)/2)-7)):
            ...
            dmrf = estimate_dMRF(strajs=[pucker_all[seq_no][:,data_index]], lag=20, Encoder=LabelBinarizer(neg_label = -1, pos_label = 1),
    ...
    with open(f'dmrf_tetramer_20_{int(k)}.dmrf', 'wb') as f:
        pickle.dump(dmrfs_, f)
    
    k += 1
    print(f'dmrf_tetramer_20_{int(k)}.dmrf complete ....')
